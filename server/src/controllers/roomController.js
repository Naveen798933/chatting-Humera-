...
export async function getRoom(req, res) {
  ...
  const messages = await Message.find({ roomId: room._id }).sort({ createdAt: 1 }).limit(200).populate('sender', 'name email avatarUrl nickname');
...
export async function sendMessage(req, res) {
  const { text, messageType = 'text', attachmentUrl = '' } = req.body;
  const room = req.room;
