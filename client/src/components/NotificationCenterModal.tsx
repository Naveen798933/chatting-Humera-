...
export const NotificationCenterModal: React.FC<NotificationCenterModalProps> = ({
  ...
  const handleNotificationClick = (notif: AppNotification) => {
    onClose();
    if ((notif.type === 'message' || notif.type === 'group_invite') && notif.data?.chatId && onSelectChat) {
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-5">
        <motion.div
          ...
          className="glass-panel w-full max-w-md rounded-3xl border border-white/10 p-5 sm:p-6 shadow-2xl flex flex-col max-h-[90dvh] overflow-hidden"
        >
          {/* Header */}
